...
origin_data = pd.read_csv("2.28도서관 장서 대출목록 (2022년 12월) (1).csv", encoding=chardet.detect(d).get('encoding')).drop("Unnamed: 13", axis=1)
web_clawled_data = pd.read_csv("pandas_output_books_price_pages.csv")
...
res_filtered_data.to_csv("filtered_merged_data.csv",index=False)
